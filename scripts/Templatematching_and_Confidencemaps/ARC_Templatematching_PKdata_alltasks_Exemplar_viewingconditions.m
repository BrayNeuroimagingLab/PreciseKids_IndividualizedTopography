function ARC_Templatematching_PKdata_alltasks_Exemplar_viewingconditions(subject, task)
...
%Create dconn
connFile = sprintf('/bulk/bray_bulk/Shefali_PreciseKIDS/newmc_matlabdir/%s_%s_allsessions.dconn.nii', subject, task);
...
sub_data=ciftiopen(sprintf('/bulk/bray_bulk/Shefali_PreciseKIDS/newmc_matlabdir/%s_%s_allsessions.dtseries.nii', subject, task), wbcommand);
sub_data.cdata=assignedNetwork;
outputFile = sprintf('/bulk/bray_bulk/Shefali_PreciseKIDS/newmc_matlabdir/%s_%s_HCPAdultChild_overlap_Exemplar_Dice.dscalar.nii', subject, task);
ciftisavereset(sub_data, outputFile, wbcommand);

% Save maximum dice map
sub_data.cdata = max_dice_map';
max_dice_file = sprintf('/bulk/bray_bulk/Shefali_PreciseKIDS/newmc_matlabdir/%s_%s_HCPAdultChild_overlap_14networkassignment_Vertexwise_Exemplar_maxdice.dscalar.nii', subject, task);
ciftisavereset(sub_data, max_dice_file, wbcommand);
    
% Save entropy map
sub_data.cdata = entropy_map';
entropy_file = sprintf('/bulk/bray_bulk/Shefali_PreciseKIDS/newmc_matlabdir/%s_%s_HCPAdultChild_overlap_14networkassignment_Vertexwise_Exemplar_entropy.dscalar.nii', subject, task);
ciftisavereset(sub_data, entropy_file, wbcommand);

% Save all dice values
sub_data.cdata = all_dice_values;
all_dice_file = sprintf('/bulk/bray_bulk/Shefali_PreciseKIDS/newmc_matlabdir/%s_%s_HCPAdultChild_overlap_14networkassignment_Vertexwise_Exemplar_alldicevalues.dscalar.nii', subject, task);
