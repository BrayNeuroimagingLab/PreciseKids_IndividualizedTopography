#!/bin/bash
#Script to open and mean center Cifti for 1 Subject
...
subjects_file=/bulk/bray_bulk/Shefali_PreciseKIDS/datalist_parent_dconn.txt #EXCEPT 003
...
SCDIR=/bulk/bray_bulk/Shefali_PreciseKIDS/spatialcorr
...
export PATH=~/matlab/cifti-matlab-master:$PATH
...
echo "Starting SpatialCorr for each parent at `date`"

matlab -nodisplay -nodesktop -nosplash -r "arc_SpatialCorrP('$SCDIR/MSCAveraged_Timeseries.dconn.nii', '$MATDIR/${subject}_${tasks}_sample1_censored.dconn.nii', '${subject}', '${tasks}')"

echo "Finished SpatialCorr for each parent at `date`"
