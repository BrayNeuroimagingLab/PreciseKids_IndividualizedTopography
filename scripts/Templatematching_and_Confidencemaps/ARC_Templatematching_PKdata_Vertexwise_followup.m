function ARC_Templatematching_PKdata_Vertexwise_followup(subject, task)
...
%Open dtseries
inputFile=sprintf('/bulk/bray_bulk/Shefali_PreciseKIDS/newmc_matlabdir/%s_%s_allsessions67.dtseries.nii', subject, task);
sub_data = ciftiopen(inputFile, wbcommand);
sub_data = sub_data.cdata;

%Remove motion
% Load temporal mask
count=1;
for ses=6:7
    temporal_mask_path{count} = sprintf('/bulk/bray_bulk/Shefali_PreciseKIDS/newmc_matlabdir/%s_ses-%d_%s_OCMASK_TEMPORAL.txt', subject, ses, task);
    temporal_mask{count} = logical(load(temporal_mask_path{count}));
    count = count + 1;
end

temporal_mask_allses=[temporal_mask{1}; temporal_mask{2}];

% Apply the temporal mask to the vertex-wise time series data
sub_data_mr = sub_data(:, temporal_mask_allses);

% Save motion removed dtseries
sub_data = ciftiopen(inputFile, wbcommand);
sub_data.cdata = sub_data_mr;
ciftisavereset(sub_data, sprintf('/bulk/bray_bulk/Shefali_PreciseKIDS/newmc_matlabdir/%s_%s_allsessions67_censored.dtseries.nii', subject, task),  wbcommand);

%Create dconn
file_path = sprintf('/bulk/bray_bulk/Shefali_PreciseKIDS/newmc_matlabdir/%s_%s_allsessions67_censored.dtseries.nii', subject, task);
connFile = sprintf('/bulk/bray_bulk/Shefali_PreciseKIDS/newmc_matlabdir/%s_%s_allsessions67_censored.dconn.nii', subject, task);
...
sub_data=ciftiopen(sprintf('/bulk/bray_bulk/Shefali_PreciseKIDS/newmc_matlabdir/%s_%s_allsessions67_censored.dtseries.nii', subject, task), wbcommand);
sub_data.cdata=binarized_sub_data;
ciftisavereset(sub_data, sprintf('/bulk/bray_bulk/Shefali_PreciseKIDS/newmc_matlabdir/%s_%s_allsessions67_censored_binarizedmap.dtseries.nii', subject, task),  wbcommand);
...
sub_data=ciftiopen(sprintf('/bulk/bray_bulk/Shefali_PreciseKIDS/newmc_matlabdir/%s_%s_allsessions67_censored_binarizedmap.dtseries.nii', subject, task), wbcommand);
sub_data.cdata=assignedNetwork;
ciftisavereset(sub_data, sprintf('/bulk/bray_bulk/Shefali_PreciseKIDS/newmc_matlabdir/%s_%s_allsessions67_MSCavgtemplatematching_14networkassignment_Vertexwise.dscalar.nii', subject, task),  wbcommand);
