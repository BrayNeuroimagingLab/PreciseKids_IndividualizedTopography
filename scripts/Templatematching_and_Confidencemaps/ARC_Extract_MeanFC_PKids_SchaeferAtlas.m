function fc_table = ARC_Extract_MeanFC_PKids_SchaeferAtlas(subject, dconn_dir, Schaefer_dscalar, distmat_file, network_labels, dist_thresh_mm)

% Inputs:
%   subject             - each sub
%   dconn_dir           - path to subject-level .dconn.nii files
%   distmat_file        - Conte distance matrix
%   network_labels      - array of network labels (Schaefer atlas parcels)
...
template_Schaefer = ciftiopen(Schaefer_dscalar, wbcommand);
...
if endsWith(subj, 'P')
    group = "Adult";
    net_labels = template_Schaefer.cdata(1:64984);
...
% Load subject dconn
dconn_file = fullfile(dconn_dir, [subj '_alltasks_sample1_censored.dconn.nii']);
...
save(fullfile(dconn_dir, [subj '_fc_table_template_Schaefer.mat']), 'fc_table');
