% FC_Vertex_Connectomes_LME_Analysis.m

%% --- Setup ---
fc_dir = '/Users/shefalirai/Downloads/FC_Vertex_Connectomes/';
...
beh_summary = varfun(@mean, beh_table, ...
    ...
    'GroupingVariables', {'sub', 'sex', 'group'});
...
for m = 1:length(methods)
    ...
    colorbar; 
    caxis([-0.1 0.1]);  % adjust as needed for visual contrast
    axis square;
    ...
    % Overlay stars for FDR-significant edges
    for r = 1:height(sig_edges)
        parts = split(sig_edges.Edge{r}, '–');
        i = find(strcmp(labels_this, parts{1}));
        j = find(strcmp(labels_this, parts{2}));
    ...
    % Optional legend for star colors
    text(nValid + 0.5, 1, '* = Adult > Child', 'Color', 'red', 'FontSize', 12);
    text(nValid + 0.5, 2, '* = Child > Adult', 'Color', [0.4 0 0.6], 'FontSize', 12);
    
    % Apply uniqueness mask to beta matrix
    uniq_crop = unique_maps{m}(validNetworks, validNetworks);
    [ui, uj] = find(uniq_crop);
    for k = 1:length(ui)
        text(uj(k), ui(k), '*', 'FontSize', 24, ...
...
for m = 1:numel(methods)
    ...
    % 4) Beta/p/q matrices (UPPER TRIANGLE ONLY; lower stays NaN)
