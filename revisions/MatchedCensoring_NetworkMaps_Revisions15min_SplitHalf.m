%% 15min split-half dtseries
% Creates two non-overlapping independent samples per subject, each = 15 min (450 vols).
%
...
TR_SECONDS          = 2;
TARGET_EACH         = round((15 * 60) / TR_SECONDS);  % 450 vols per sample
TARGET_TOTAL        = TARGET_EACH * 2;                 % 900 vols to draw from pool
...
output_dir = fullfile(getenv('HOME'), 'Desktop', '15min_splitHalf_dtseries');
...
for sub = 2:26
    ...
    for is_parent = 0:1
        ...
        if n_pool < TARGET_TOTAL
            fprintf('    SKIPPING %s: only %d good vols, need %d for 15min split-half\n', ...
                    subject, n_pool, TARGET_TOTAL);
            clear pooled_data
            continue;
        end

        % ── Randomly draw TARGET_TOTAL, split into two non-overlapping halves ──
        seed = 150000 + 1000 * sub + is_parent;
        rs   = RandStream('mt19937ar', 'Seed', seed);
        sel  = sort(randsample(rs, n_pool, TARGET_TOTAL));  % 900 unique indices

        sample1_data = pooled_data(:, sel(1:TARGET_EACH));           % vols 1-450
        sample2_data = pooled_data(:, sel(TARGET_EACH+1:end));       % vols 451-900
        clear pooled_data
        ...
        for samp = 1:2
            ...
            out_file = fullfile(output_dir, ...
                sprintf('sub-19730%02d%s_alltasks_15min_sample%d.dtseries.nii', sub, subtype, samp));
            try
...
%% Summary check
fprintf('\n=== 15min Split-Half Summary ===\n');
for samp = 1:2
    files = dir(fullfile(output_dir, sprintf('*15min_sample%d.dtseries.nii', samp)));
