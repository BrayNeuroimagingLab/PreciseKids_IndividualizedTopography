function ARC_ConfidenceMaps_PKdata_alltasks_Sample2(subject, task)
...
%Create dconn
file_path = sprintf('/bulk/bray_bulk/Shefali_PreciseKIDS/newmc_matlabdir/%s_%s_sample2_censored.dtseries.nii', subject, task);
connFile = sprintf('/bulk/bray_bulk/Shefali_PreciseKIDS/newmc_matlabdir/%s_%s_sample2_censored.dconn.nii', subject, task);
...
for vertex = 1:numVertices
        ...
        % Calculate entropy
        eta_norm = etaToTemplateVertex / sum(etaToTemplateVertex);
        entropy_map(vertex) = -sum(eta_norm .* log2(eta_norm + eps));
end


% Save maximum eta map
sub_data=ciftiopen(sprintf('/bulk/bray_bulk/Shefali_PreciseKIDS/newmc_matlabdir/%s_%s_sample2_censored.dtseries.nii', subject, task), wbcommand);
max_eta_output = sub_data;
max_eta_output.cdata = max_eta_map;
max_eta_file = sprintf('/bulk/bray_bulk/Shefali_PreciseKIDS/newmc_matlabdir/%s_%s_sample2_templatematching_maxeta.dscalar.nii', subject, task);
ciftisavereset(max_eta_output, max_eta_file, wbcommand);
    
% Save entropy map
entropy_output = sub_data;
entropy_output.cdata = entropy_map;
entropy_file = sprintf('/bulk/bray_bulk/Shefali_PreciseKIDS/newmc_matlabdir/%s_%s_sample2_templatematching_entropy.dscalar.nii', subject, task);
