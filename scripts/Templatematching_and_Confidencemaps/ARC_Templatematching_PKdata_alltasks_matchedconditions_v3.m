...
for networks = 1:length(networkList)
    ...
    msc{networks} = ciftiopen(sprintf('/bulk/bray_bulk/Shefali_PreciseKIDS/newmc_matlabdir/dworetsky-hcp_Dworetsky-HCP-network%d_network_probability_91282vertices_0.2thresholded.dscalar.nii', networkNum), wbcommand);
...
outputFile = sprintf('/bulk/bray_bulk/Shefali_PreciseKIDS/newmc_matlabdir/%s_%s_DworetskyHCPAdult_0.2ProbabilityThresh_14networkassignment_Vertexwise_sample1_matchedconditions_Dice.dscalar.nii', subject, task);
ciftisavereset(sub_data, outputFile, wbcommand);

% Save maximum dice map
sub_data.cdata = max_dice_map';
max_dice_file = sprintf('/bulk/bray_bulk/Shefali_PreciseKIDS/newmc_matlabdir/%s_%s_DworetskyHCPAdult_0.2ProbabilityThresh_14networkassignment_Vertexwise_sample1_matchedconditions_maxdice.dscalar.nii', subject, task);
ciftisavereset(sub_data, max_dice_file, wbcommand);
    
% Save entropy map
sub_data.cdata = entropy_map';
entropy_file = sprintf('/bulk/bray_bulk/Shefali_PreciseKIDS/newmc_matlabdir/%s_%s_DworetskyHCPAdult_0.2ProbabilityThresh_14networkassignment_Vertexwise_sample1_matchedconditions_entropy.dscalar.nii', subject, task);
