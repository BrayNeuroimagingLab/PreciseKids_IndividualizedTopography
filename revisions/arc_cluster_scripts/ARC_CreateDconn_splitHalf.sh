#!/bin/bash
# Create dconn for split-half samples.
...
echo "Starting CreateDconn split-half at $(date)"

matlab -nodisplay -nodesktop -nosplash -r \
  "arc_create_dconn_splitHalf('$MATDIR/${subject}_${task}_${DURATION}_sample${SAMPLE}.dtseries.nii', \
  '${subject}', '${task}', '${DURATION}', '${SAMPLE}')"

echo "Finished CreateDconn split-half at $(date)"
